...
function toAbsoluteOnServer(input: RequestInfo | URL): RequestInfo | URL {
  ...
  let origin = process.env.APP_ORIGIN || (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : "http://localhost:3000");
...
export async function httpJson<T>(input: RequestInfo | URL, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    const body: ApiErrorBody | undefined = parsed && typeof parsed === "object" ? (parsed as ApiErrorBody) : undefined;
