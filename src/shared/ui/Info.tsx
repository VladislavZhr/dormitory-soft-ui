...
export default function Info({ label, value, edit = false, type = "text", wide = false, onChange }: Props) {
  return (
    <div className={`flex flex-col ${wide ? "col-span-2" : ""}`}>
      ...
      {edit ? (
        ...
        <span className="mt-1 rounded-lg bg-slate-50 px-2 py-1 text-sm text-slate-900">{value || "—"}</span>
      )}
    </div>
  );
}
