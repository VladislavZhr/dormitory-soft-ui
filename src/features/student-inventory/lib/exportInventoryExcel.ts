...
export async function exportInventoryExcel(args: {
  ...
  for (let i = 0; i < ITEM_ORDER.length; i++) {
    ...
    for (let c = 1; c <= 14; c++) {
      ...
      cell.border = cellBorder;
      cell.alignment = c === 2 ? { vertical: "middle", horizontal: "left" } : { vertical: "middle", horizontal: "center" };
    }
  ...
  const outName = filename ?? `Арматурний_список_№2_${(student.fullName || "").replace(/\s+/g, "_")}_кімн_${String(roomNoOverride ?? student.roomNumber ?? "")}.xlsx`;
